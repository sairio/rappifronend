export class AddressService {
  ...
  submit(stContent, stLat, stLng, stAddressId, stAddress, stStoreType) {
    ...
    if (isValid) {
      ...
      //manage google geocoder
      let currentUrl = this.serverUrl;
      isValid = this.geocoder(currentUrl, stContent, stLat, stLng, stAddressId, stAddress, stStoreType);
    }
  }
  ...
  /**
   * Realiza el todo el procesamiento concerniente al Geocoder de google map
   * @param currentUrl 
   * @param stContent 
   * @param stLat 
   * @param stLng 
   ...
   * @param stStoreType 
   */
  geocoder(currentUrl, stContent, stLat, stLng, stAddressId, stAddress,stStoreType)
  {
    let isValid = true;
    let currentCountry = localStorage.getItem(stContent) ? localStorage.getItem(stContent).countryName : ENV.location[0].name;
      ...
      addressComponents.forEach((component) => {
        ...
        if (componentType[0] === 'country') {
          ...
          if ((currentCountry === this.selectedCountry) || (this.selectedCountry === 'Mexico')) {
            ...
            return this.sendCoord(currentUrl, lat, lng, stLat, stLng, stAddressId, stAddress,stStoreType)
          } else {
   ...
   * @param stStoreType 
   */
  saveAddress(stLat, stLng, stAddressId, stAddress,stStoreType)
  {
    ...
    newAddress.save().then(function (result) {
      ...
      if (this.flow === 'checkout') {
        this.checkoutCart(result, stLat, stLng, stAddressId, stAddress,stStoreType);
        this.currentlyLoading = false;
      } else if (this.flow === 'profile') {
        this.showPopUp = false;
      } else if (this.flow === 'login' || this.flow === 'change-direction') {
        this.resultLogin = result;
        this.showReloadWarning = this.verifyReloadWarning(result, stLat, stLng, stStoreType);
      }
    });
  }

  /**
   * Comprueba las coordenadas del carrito de compras.
   * @param result 
   * @param stLat 
   * @param stLng 
   * @param stStoreType 
   */
  verifyReloadWarning (result, stLat, stLng, stStoreType)
  {
    let addLat = result.lat ? Math.abs(Number(result.lat)) : 0;
    let addLng = result.lng ? Math.abs(Number(result.lng)) : 0;
    let storageLat = localStorage.getItem(stLat) ? Math.abs(parseInt(localStorage.getItem(stLat))) : 0;
    ...
    if ((Math.abs(addLat - storageLat) > 0.000001 || Math.abs(addLng - storageLng) > 0.000001) && 
        this.cart.getCart(localStorage.getItem(stStoreType)) && 
        this.cart.getCart(localStorage.getItem(stStoreType)).cartItems.length !== 0) {
      return true;
    } else {
      return false;
    }
  ...
  /**
   * Realiza el proceso de checkout en el carrito de compras
   * @param result 
   * @param stLat 
   * @param stLng 
   * @param stAddressId 
   ...
   * @param stStoreType 
   */
  checkoutCart(result, stLat, stLng, stAddressId, stAddress, stStoreType)
  {
    let storeType = localStorage.getItem(stStoreType);
    let id = result['id'];
    let address = result.address;
  ...
  /**
   * Realiza el envio via ajax de los datos correspondienets a geolocalizacion de google map
   * @param url 
   * @param lat 
   * @param lng 
   * @param stLat 
   ...
   * @param stStoreType 
   */
  sendCoord(url,lat, lng, stLat, stLng, stAddressId, stAddress,stStoreType) {
    this.$.ajax({
      type: "GET",
      url: `${url}${this.ENV.searchStore}lat=${lat}&lng=${lng}`,
    }).then(() => {
      this.saveAddress(stLat, stLng, stAddressId, stAddress,stStoreType);
    }).fail((err) => {
      this.currentlyLoading = false;
